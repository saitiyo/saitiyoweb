	...
	return (
		<main className="inventory-page">
			<header className="inventory-header flex justify-between mb-6">
				...
				<Link className="relative z-10 inline-flex items-center justify-center whitespace-nowrap rounded-lg !bg-black px-4 py-2.5 text-sm font-semibold !text-white transition hover:!bg-[#333]" href={`/site/${siteId}/inventory/addinventory`}>Add Item</Link>
			</header>

			<section className="inventory-toolbar mb-5 flex flex-col gap-3 rounded-2xl border border-[#ddd] bg-white p-3 shadow-[0_5px_16px_rgba(0,0,0,0.05)] lg:flex-row lg:items-center lg:justify-between" aria-label="Inventory controls">
				<label className="search-box flex min-h-12 flex-1 items-center gap-3 rounded-xl border border-[#ddd] bg-[#f5f5f5] px-4 text-[#111] transition focus-within:border-[#111] focus-within:bg-white focus-within:ring-4 focus-within:ring-[#eee] lg:max-w-3xl"><Search size={20} aria-hidden="true" /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search products by name or category..." aria-label="Search products" className="min-w-0 flex-1 bg-transparent text-sm text-[#111] outline-none placeholder:text-[#777]" />{search && <button type="button" onClick={() => setSearch("")} aria-label="Clear search" className="flex text-[#111] transition hover:text-[#555]"><CircleX size={17} /></button>}</label>
				<button className="sort-button flex min-h-12 items-center justify-between gap-3 rounded-xl border border-[#ddd] bg-white px-4 text-sm font-semibold text-[#111] transition hover:border-[#111] hover:bg-[#f5f5f5] lg:min-w-44" type="button"><span className="text-[11px] font-semibold uppercase tracking-wide text-[#666]">Sort by</span> Recent <ChevronDown size={16} className="text-[#111]" /></button>
			</section>

			<div className="filter-row mb-8 flex flex-wrap items-center gap-1.5 rounded-xl border border-[#ddd] bg-white p-1.5 shadow-sm" role="tablist" aria-label="Filter inventory">
				<span className="hidden px-3 text-[10px] font-bold uppercase tracking-[0.14em] text-[#666] sm:inline">View</span>
				{filters.map((filter) => <button className={`filter-chip flex items-center gap-2 rounded-lg border px-3.5 py-2 text-xs font-bold transition ${activeFilter === filter ? "active border-black bg-black !text-white shadow-[0_3px_8px_rgba(0,0,0,0.16)]" : "border-transparent bg-transparent text-[#555] hover:bg-[#f2f2f2] hover:text-[#111]"}`} key={filter} onClick={() => setActiveFilter(filter)} role="tab" aria-selected={activeFilter === filter} type="button">{filter}{filter !== "All" && <span className={`filter-count rounded-md px-1.5 py-0.5 text-[10px] ${activeFilter === filter ? "bg-white/20 !text-white" : "bg-[#eee] text-[#555]"}`}>{filter === "Low stock" ? lowStockCount : filter === "Out of stock" ? outOfStockCount : inventory.filter((item) => item.status === "In stock").length}</span>}</button>)}
			</div>
			...
			<section className="products-section mb-6">
				<div className="section-heading" style={{ paddingBottom: "16px" }}><div><p className="eyebrow">Current catalogue</p><h2>{activeFilter === "All" ? "All products" : activeFilter}</h2></div><span>{visibleItems.length} items</span></div>
				<div className="product-list grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-3" style={{ marginTop: "32px" }}>
					{visibleItems.map((item) => <article className="product-row inventory-product-card relative flex min-h-[214px] flex-col justify-between rounded-2xl border border-[#d9e2e6] bg-white p-5 shadow-[0_5px_16px_rgba(31,52,64,0.08)] transition hover:-translate-y-1 hover:border-[#b8c8cf] hover:shadow-[0_12px_24px_rgba(31,52,64,0.14)]" key={item.id}><div className="product-card-top flex items-start justify-between"><div className="product-icon flex h-14 w-14 items-center justify-center rounded-xl text-[#74818b]" style={{ backgroundColor: item.accent }}><Box size={27} strokeWidth={1.7} /></div><Link className="row-action !bg-black !text-white flex h-8 w-8 items-center justify-center rounded-full transition hover:!bg-[#333] hover:!text-white" href={`/site/${siteId}/inventory/inventorydetails?itemId=${encodeURIComponent(String(item.id))}`} aria-label={`Open ${item.name}`}><ArrowRight size={18} /></Link></div><div className="product-main mt-5"><h3 className="text-lg font-bold tracking-[-0.025em] text-[#17232d]">{item.name}</h3><p className="mt-1.5 text-xs text-[#87949d]">{item.category} <span className="px-1">·</span> {item.quantity} {item.unit}</p></div><div className="product-stock mt-5 flex items-center justify-between gap-3 border-t border-[#edf1f3] pt-4"><strong className="text-[15px] font-bold text-[#17232d]">{formatPrice(item.price)}</strong><span className={`stock-pill rounded-full px-2.5 py-1 text-[10px] font-bold ${item.status === "In stock" ? "bg-[#e8f2db] text-[#527235]" : item.status === "Low stock" ? "bg-[#fff0d9] text-[#936a31]" : "bg-[#fae5e5] text-[#a1484b]"}`}>{item.status === "In stock" ? `${item.stock} in stock` : item.status}</span></div></article>)}
					{visibleItems.length === 0 && <div className="empty-state"><Search size={24} /><strong>No Items found</strong><span>Try a different search or filter.</span></div>}
				</div>
			</section>
...
<style jsx>{`
	.inventory-page { --ink: #111; --muted: #666; --line: #ddd; min-height: calc(100vh - 84px); background: #fff; color: var(--ink); padding: 32px clamp(24px, 4vw, 64px) 48px; }
	...
	.inventory-header { margin-bottom: 26px; } .eyebrow { color: #666; font-size: 11px; font-weight: 700; letter-spacing: .13em; margin: 0 0 7px; text-transform: uppercase; } h1, h2, h3, p { margin: 0; } h1 { font-size: clamp(30px, 4vw, 44px); letter-spacing: -.04em; line-height: 1; } h2 { font-size: 22px; letter-spacing: -.03em; text-transform: capitalize; }
	.site-switcher, .sort-button { align-items: center; background: #fff; border: 1px solid var(--line); border-radius: 10px; color: var(--ink); display: flex; gap: 11px; padding: 10px 14px; } .site-switcher { min-width: 210px; text-align: left; } .site-mark { align-items: center; background: #f0f0f0; border-radius: 8px; color: #111; display: flex; height: 32px; justify-content: center; width: 32px; } .site-switcher-copy { display: flex; flex: 1; flex-direction: column; font-size: 13px; font-weight: 700; gap: 2px; } .site-switcher-copy small, .sort-button span { color: var(--muted); font-size: 10px; font-weight: 600; }
	.inventory-toolbar { align-items: stretch; background: #fff; border: 1px solid var(--line); border-radius: 16px; box-shadow: 0 4px 14px rgba(0, 0, 0, .04); margin-bottom: 14px; padding: 10px; } .search-box { align-items: center; background: #f5f5f5; border: 1px solid #ddd; border-radius: 10px; color: #111; display: flex; flex: 1; gap: 12px; max-width: 660px; padding: 12px 14px; } .search-box input { border: 0; color: var(--ink); flex: 1; font: inherit; font-size: 14px; outline: 0; } .search-box input::placeholder { color: #777; } .search-box button { background: transparent; border: 0; color: #111; display: flex; padding: 0; } .sort-button { border: 1px solid #ddd; font-size: 13px; gap: 8px; } .sort-button span { margin-right: 4px; }
	.filter-row { align-items: center; background: #fff; border: 1px solid var(--line); border-radius: 12px; display: flex; flex-wrap: wrap; gap: 6px; margin-bottom: 25px; padding: 6px; } .filter-chip { align-items: center; background: transparent; border: 1px solid transparent; border-radius: 7px; color: #555; display: flex; font-size: 12px; font-weight: 700; gap: 8px; padding: 8px 13px; } .filter-chip:hover { background: #f5f5f5; color: #111; } .filter-chip.active { background: #000; border-color: #000; color: #fff !important; box-shadow: 0 3px 8px rgba(0, 0, 0, .15); } .filter-chip.active .filter-count { background: rgba(255, 255, 255, .18); color: #fff !important; } .filter-count { background: #eee; border-radius: 5px; font-size: 10px; min-width: 17px; padding: 2px 5px; text-align: center; }
	...
	.products-section { margin-top: 24px; max-width: 1100px; } .section-heading { margin-bottom: 0; } .section-heading > span { color: var(--muted); font-size: 12px; font-weight: 600; } .product-list { display: grid; gap: 14px; grid-template-columns: repeat(3, minmax(0, 1fr)); margin-top: 34px; } .product-row { background: #fff; border: 1px solid var(--line); border-radius: 16px; box-shadow: 0 4px 12px rgba(0, 0, 0, .045); display: grid; gap: 20px; min-height: 214px; padding: 18px; transition: border-color .2s ease, box-shadow .2s ease, transform .2s ease; } .product-row:hover { border-color: #111; box-shadow: 0 10px 22px rgba(0, 0, 0, .09); transform: translateY(-2px); } .product-icon { align-items: center; border-radius: 12px; color: #111; display: flex; height: 58px; justify-content: center; width: 58px; } .product-main { min-width: 0; } .product-main h3 { font-size: 17px; letter-spacing: -.025em; } .product-main p { color: #666; font-size: 12px; margin-top: 6px; } .product-main p span { padding: 0 4px; } .product-stock { align-items: center; border-top: 1px solid #ddd; display: flex; justify-content: space-between; gap: 10px; padding-top: 14px; } .product-stock strong { font-size: 15px; } .stock-pill { border-radius: 99px; font-size: 11px; font-weight: 700; padding: 5px 10px; white-space: nowrap; } .in-stock { background: #e8f2db; color: #527235; } .low-stock { background: #fff0d9; color: #936a31; } .out-of-stock { background: #fae5e5; color: #a1484b; } .row-action { align-items: center; background: #f0f0f0; border: 0; border-radius: 50%; color: #111; display: flex; height: 32px; justify-content: center; padding: 0; position: absolute; right: 16px; top: 18px; width: 32px; } .product-row { position: relative; } .empty-state { align-items: center; background: #fff; border: 1px dashed var(--line); border-radius: 14px; color: var(--muted); display: flex; flex-direction: column; gap: 8px; padding: 45px; } .empty-state strong { color: var(--ink); }
